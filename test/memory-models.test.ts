...
console.assert(
  !requiresAggregation(personalFamily),
  'Personal family should not require aggregation'
);
console.assert(requiresAggregation(cohortFamily), 'Cohort family should require aggregation');
console.assert(
  requiresAggregation(populationFamily),
  'Population family should require aggregation'
);
...
console.assert(
  !validateRecallQuery({ session_id: 'session-456' }),
  'Missing hashed_pseudonym should fail'
);
...
console.assert(
  validateForgetRequest({ hashed_pseudonym: 'hs_dXNlci0xMjNfaGFzaGVkX3BzZXVkb255bV90ZXN0' }),
  'Hashed pseudonym only should pass'
);
...
console.assert(
  baseResponse.audit_receipt_id === 'receipt-abc-123',
  'Audit receipt ID should match'
);
console.assert(typeof baseResponse.timestamp === 'string', 'Timestamp should be string');
console.assert(
  new Date(baseResponse.timestamp).getTime() > 0,
...
console.assert(
  allErrorCodes.includes(ErrorCode.VALIDATION_ERROR),
  'Should include VALIDATION_ERROR'
);
...
console.assert(
  allErrorCodes.includes(ErrorCode.SERVICE_UNAVAILABLE),
  'Should include SERVICE_UNAVAILABLE'
);
